...
namespace Elara.WoW.Helpers
{
    public class SpellInfo
    {
        ...
        public bool IsOnActionBar =>
            GameOwner.ActionBar.GetActiveSlots().Any(x => x.Type == WowActionButtonType.Spell && x.SpellId == SpellId);

        public bool CheckSpellAttribute(WowSpellAttributes p_Attribute)
        {
        ...
        public bool RequireTargetFacingCaster => CheckSpellAttribute(WowSpellAttributes.REQ_TARGET_FACING_CASTER);

        public bool RequireCasterBehingTarget => CheckSpellAttribute(WowSpellAttributes.REQ_CASTER_BEHIND_TARGET);

        public bool RequireFacingTarget => SpellCastingRequirements.Any(x => x.FacingCasterFlags != 0);

        public bool IsTalent => CheckSpellAttribute(WowSpellAttributes.IS_TALENT);

        public bool DontBreakStealth => CheckSpellAttribute(WowSpellAttributes.DONT_BREAK_STEALTH);
